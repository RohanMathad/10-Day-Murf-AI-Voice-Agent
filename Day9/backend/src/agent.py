...
# -------------------------
# Simple Product Catalog (Amazon India style, pronounceable names)
# -------------------------
# Compact pronounceable catalog with attributes: id, name, description, price (INR), category, color, sizes
CATALOG = [
    {
        "id": "mug-001",
    ...
    },
    {
        ...
        "description": "Double wall steel water bottle.",
        "price": 599,
        "currency": "INR",
        "category": "bottle",
        "color": "silver",
    ...
    },
    {
        ...
        "description": "True wireless earbuds with microphone.",
        "price": 900,
        "currency": "INR",
    ...
    },
    {
        ...
        "description": "1 meter USB Type C charging cable.",
        "price": 199,
        "currency": "INR",
    ...
    },
    {
        ...
        "description": "Insulated lunch box for hot meals.",
        "price": 699,
        "currency": "INR",
        "category": "kitchen",
        "color": "blue",
    ...
    },
    {
        "id": "notebook-001",
        "name": "Notebook Pack",
        "description": "Pack of three ruled notebooks.",
        "price": 149,
        "currency": "INR",
        "category": "stationery",
        "color": "multi",
...
def find_product_by_ref(ref_text: str, candidates: Optional[List[Dict]] = None) -> Optional[Dict]:
    """Resolve references like 'second mug' or 'black bottle' to a product dict."""
    ...
    # fallback: if user said 'second item' but we couldn't match earlier, try overall cand ordinals
    for word, idx in ordinals.items():
...
def create_order_object(line_items: List[Dict], currency: str = "INR") -> Dict:
    """line_items: [{product_id, quantity, attrs}] Returns an order dict (id, items, total, currency, created_at)"""
    items = []
    total = 0
...
async def show_catalog(
    ...
    lines.append("You can say: 'Add the second item to my cart' or 'add mug-001 to my cart, quantity 2'.")
    if any(p.get('category') == 'mobile' for p in prods):
        lines.append("To buy a phone say: 'Add phone-001 to my cart'.")
    return "\n".join(lines)
...
# -------------------------
# The Agent (Shopkeeper persona)
# -------------------------
class GameMasterAgent(Agent):
    def __init__(self):
        # System instructions describe the shopkeeper persona and commerce role
        instructions = """
        You are a friendly shopkeeper and voice assistant for a small online shop.
        Universe: A compact product catalog with simple items for daily use.
        Tone: Warm and helpful, keep sentences short for TTS clarity.
        ...
        Rules:
            ...
            - When presenting options, include product id and price (e.g. 'mug-001 — 350 INR').
        """
        super().__init__(
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT — Shopkeeper")
